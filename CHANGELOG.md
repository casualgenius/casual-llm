...
### Breaking Changes
- The introduction of the ChatOptions moved response_format, max_tokens, tools, temperature from params on the provider and model `chat` & `stream` method params to inside the `options` param. If you were using these you will need to update your calls.
- `ollama` was moved to an optional dependency, if you use ollama you may need to add it to your project dependencies specifically or add this library with `casual-llm[ollama]`
